...
def decrypt() -> None:
    ...
    for pathStr, keyHex in keys.items():
        ...
        try:
            plaintext = decrypteBytes(cipherText, key)
        except Exception as e:
            print(f"\n{ERROR} decryption incomplete | {f.name}: {e} (fail) ")
            continue
        f.write_bytes(plaintext)
        postDeploy = dataEntropy(plaintext)
        print(f"\n{NOTIFICATION} [dec] {f.name<20} current entropy: {postDeploy:.4f} bites/bytes")
        restore += 1
...
def main() -> None:
    ...
    parser.add_argument("--restore", action="storeTrue")
    parser.add_argument("--dry-run", action="storeTrue")
