...
def targetFiles() -> list[Path]:
    if not dummyDir.is_dir():
        print(f"{ERROR} {dummyDir} does not exist")
        sys.exit(1)
...
def decrypt() -> None:
    ...
    if stateFile.exists():
        ...
        if state != "encrypted":
            print(f"{WARNING} Files are not currently encrypted (state: {state}).")
            print(f"{WARNING} Try: 'python3 dataImpact.py --dry-run' to check entropy.")
            sys.exit(1)
    ...
    for pathStr, keyHex in keys.items():
        ...
        if not f.exists():
            ...
            print(f"{WARNING} Try: Check if files have been deleted or restored")
            continue
